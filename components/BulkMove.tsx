...
export function BulkMove({
    ...
    const fetchFolders = async (path: string) => {
        ...
        try {
            ...
            const folders = (data.items ?? [])
                .filter((item: any) => item.type === 'folder' && !selectedKeys.includes(item.key))
    ...
    const moveClicked = async () => {
        const items = selectedKeys.map(key => {
            ...
            const name = isFolderKey
                ? key.slice(0, -1).split('/').pop() + '/'
                : key.split('/').pop() || key;
            return { sourceKey: key, destinationKey: browsePath + name };
        });

        const sameDest = items.find(i => i.sourceKey === i.destinationKey);
        if (sameDest) {
            toast.error("Source and destination must be different for all items");
            return;
        }

        setIsMoving(true);
        const toastId = toast.loading(`Preparing to move ${selectedKeys.length} items...`);
        try {
                ...
                pollRef.current = setInterval(async () => {
                    try {
                        ...
                        if (job.status === 'completed') {
                            ...
                            toast.success(`Moved ${selectedKeys.length} items successfully`, {
                                id: toastId,
                            ...
                            toast.loading(`Moving ${selectedKeys.length} items...`, {
                                id: toastId,
    ...
    return (
        <>
            <Button
                variant="outline"
                className={`gap-2 flex-1 sm:flex-initial ${className || ''}`}
                onClick={openDialog}
            >
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Select destination for {selectedKeys.length} items.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <DialogFooter>
                        <Button onClick={moveClicked} disabled={isMoving}>
